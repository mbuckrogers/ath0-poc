...
public class GatewayAuthenticationProvider implements AuthenticationProvider, InitializingBean {
	...
	public Authentication authenticate(final Authentication authentication) throws AuthenticationException {
		...
		try {
			...
			throw new Auth0TokenException(
					"InvalidKeyException thrown while decoding JWT token " + e.getLocalizedMessage());
		} catch (NoSuchAlgorithmException e) {
			throw new Auth0TokenException(
					"NoSuchAlgorithmException thrown while decoding JWT token " + e.getLocalizedMessage());
		} catch (IllegalStateException e) {
			throw new Auth0TokenException(
					"IllegalStateException thrown while decoding JWT token " + e.getLocalizedMessage());
		} catch (SignatureException e) {
			throw new Auth0TokenException(
					"SignatureException thrown while decoding JWT token " + e.getLocalizedMessage());
		} catch (IOException e) {
			throw new Auth0TokenException("IOException thrown while decoding JWT token " + e.getLocalizedMessage());
		} catch (JWTVerifyException e) {
			throw new Auth0TokenException(
					"JWTVerifyException thrown while decoding JWT token " + e.getLocalizedMessage());
		}
	}
	...
	public void afterPropertiesSet() throws Exception {
		...
		case HS384:
		case HS512:
			// Auth0 Client Secrets are currently Base64 encoded, Resource
			// Server Signing Secret is NOT Base64 encoded
		...
		case RS512:
			...
			try {
				final PublicKey publicKey = pkUtil.createPublicKey(publicKeyPath);
	...
	public void setPkUtil(PublicKeyUtil pkUtil) {
		this.pkUtil = pkUtil;
	}
}
